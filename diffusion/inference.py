...
from diffusion.sampler import SamplerBase
from diffusion.schedule import ScheduleBase
from diffusion.util import t_to_alpha_sigma, tensor_slerp_2D
...
class Inference:
    ...
    def generate_unconditional(self, batch_size: int = 1, steps: int = 25, callback=None):
    ...
    def generate_variation(self, batch_size: int = 1, steps: int = 25, audio_input: torch.Tensor = None, noise_level: float = 0.7, callback = None):
        """
            Invariants:
    ...
    def generate_interpolation(self, samples: int, steps: int, audio_source: torch.Tensor, audio_target: torch.Tensor, noise_level: float = 1.0, callback=None):
        """
            Invariants:
        ...
        audio_output = []
        audio_output.append(x_0[0])#audio_output.append(self.sampler.sample(x_T[0][None, :, :], ts).squeeze(dim=0))
        
        for sample in range(samples - 2):
            audio_output.append(self.sampler.sample(tensor_slerp_2D(x_T[0], x_T[1], (1 + sample) / (samples - 1))[None, :, :], ts).squeeze(dim=0))

        audio_output.append(x_0[1])#audio_output.append(self.sampler.sample(x_T[1][None, :, :], ts).squeeze(dim=0))
