...
def create_bot_with_commands(manager: SyncManager, request_queue: mp.Queue, response_queue: mp.Queue, args, config: BotConfig, models_metadata: ModelsMetadata):
    bot = discord.Bot()
    ...
    async def regenerate(ctx: discord.commands.context.ApplicationContext, message: discord.message.Message):
        if (
            ...
            await ctx.respond(
                "_Error: **Regenerate...** can only be performed on generations made by this bot._",
                ephemeral=True,
            )
            return
        ...
        for field in embed.fields:
            if field.name == "Model":
                ...
                length_multiplier = (
                    int(field.value) if field.value != "Full Length" else -1
                )
        ...
        if length_multiplier is not None:
            await ctx.respond(
                "Error: **Regenerate...** can only be performed on generations, not variations.",
                ephemeral=True,
            )
            return
    ...
    async def generate_variation(
        ...
        if message.attachments is None:
            await ctx.respond(
                "_Error: You must choose a `.wav` file to generate a variation of._",
                ephemeral=True,
            )
            return
        ...
        if len(message.attachments) == 0:
            await ctx.respond(
                "_Error: You can only generate variations of `.wav` files._",
                ephemeral=True,
            )
            return
        ...
        if len(message.attachments) > 1:
            ...
            await ctx.respond(
                "_Error: You can only generate variations of one `.wav` file._",
                ephemeral=True,
            )
            return
        ...
        if file.size > max_file_size_bytes:
            await ctx.respond(
                f"Error: File too large. WAV must be less than {max_file_size_bytes} bytes",
                ephemeral=True,
            )
            return
    ...
    async def generate(
        ...
        original_message = await ctx.respond(f"Your request has been queued.")
